...
class BookAdapterx5(private val context: Context, private var bookList: List<book>) :
    RecyclerView.Adapter<BookAdapterx5.BookViewHolder>() {
        ...
        db.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for (postSnapshot in dataSnapshot.children) {
                    ...
                    if ((book?.bookquantity as? Int ?: 0) >= 0) { // Skip books with bookquantity less than or equal to 0

                        bookList.add(book!!)
                    }
                }
                val sortedBookList = bookList.sortedBy { it.bookyear }
